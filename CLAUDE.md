...
- Three-layer context compaction (defaults: `AGENT_MAX_CONTEXT_TOKENS=200000`, `AGENT_CONTEXT_COMPRESS_RATIO=0.5`, `AGENT_EMERGENCY_COMPACT_RATIO=0.95`):
...
- **Latency observability**: `_stream_one_completion` measures wall-clock TTFT (time from request-sent to first payload-bearing chunk) and total round duration. Surfaced via four SSE events:
  - `thinking` carries `messages_count` and `estimated_input_tokens` so the size of the prompt being sent is visible *before* TTFT.
  - `first_token` fires the moment the first real chunk arrives — `ttft_ms` is the gap that "卡在 thinking" used to look mysterious.
  - `model_routed` and `finish` both carry `ttft_ms`, `duration_ms`, and `streaming_ms` (= `duration_ms - ttft_ms`).
  Per-round timing is also persisted into the trajectory record's `latency` field so post-hoc analysis can spot which round was slow.
